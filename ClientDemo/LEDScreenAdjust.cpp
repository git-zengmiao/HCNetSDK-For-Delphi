...
LEDScreenAdjust::LEDScreenAdjust(CWnd* pParent /*=NULL*/)
	...
	, m_lGreenPermil(0)
	, m_lBluePermil(0)
    , m_bDelFullScreenGamut(FALSE)
    , m_bDelLightPanelGamut(FALSE)
    , m_bDelLightPanelWhiteBalance(FALSE)
{
}
...
void LEDScreenAdjust::DoDataExchange(CDataExchange* pDX)
{
    ...
    DDX_Control(pDX, IDC_COMB_SCREEN_CHECK_TYPE, m_CombScreenCheckType);
    DDX_Check(pDX, IDC_CHECK_DEL_FULL_SCREEN_GAMUT, m_bDelFullScreenGamut);
    DDX_Check(pDX, IDC_CHECK_DEL_LIGHT_PANEL_GAMUT, m_bDelLightPanelGamut);
    DDX_Check(pDX, IDC_CHECK_DEL_LIGHT_PANEL_WHITE_BALANCE, m_bDelLightPanelWhiteBalance);
}
...
BEGIN_MESSAGE_MAP(LEDScreenAdjust, CDialog)
    ...
    ON_BN_CLICKED(IDC_CHECK_DEL_FULL_SCREEN_GAMUT, &LEDScreenAdjust::OnBnClickedCheckDelFullScreenGamut)
    ON_BN_CLICKED(IDC_CHECK_DEL_LIGHT_PANEL_GAMUT, &LEDScreenAdjust::OnBnClickedCheckDelLightPanelGamut)
    ON_BN_CLICKED(IDC_CHECK_DEL_LIGHT_PANEL_WHITE_BALANCE, &LEDScreenAdjust::OnBnClickedCheckDelLightPanelWhiteBalance)
END_MESSAGE_MAP()
...
BOOL LEDScreenAdjust::OnInitDialog()
{
    ...
    m_CombScreenCheckType.SetCurSel(0);
...
void LEDScreenAdjust::OnBnClickedBtnOperate()
{
    ...
    if (0 == m_CombScreenCheckType.GetCurSel())
    {
        m_struScreenCheck.byScreenCheckType = 0;
    }
    if (1 == m_CombScreenCheckType.GetCurSel())
    {
        m_struScreenCheck.byScreenCheckType = 1;
        m_struScreenCheck.struAdvanceScreenCheckParam.byDelFullScreenGamut = m_bDelFullScreenGamut;
        m_struScreenCheck.struAdvanceScreenCheckParam.byDelLightPanelGamut = m_bDelLightPanelGamut;
        m_struScreenCheck.struAdvanceScreenCheckParam.byDelLightPanelWhiteBalance = m_bDelLightPanelWhiteBalance;
    }
...
void LEDScreenAdjust::OnBnClickedCheckDelFullScreenGamut()
{
    // TODO:  在此添加控件通知处理程序代码
    m_bDelFullScreenGamut = TRUE;
}


void LEDScreenAdjust::OnBnClickedCheckDelLightPanelGamut()
{
    // TODO:  在此添加控件通知处理程序代码
    m_bDelLightPanelGamut = TRUE;
}


void LEDScreenAdjust::OnBnClickedCheckDelLightPanelWhiteBalance()
{
    // TODO:  在此添加控件通知处理程序代码
    m_bDelLightPanelWhiteBalance = TRUE;
}
