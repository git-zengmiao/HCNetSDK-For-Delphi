...
void CDlgUHFBasicInfo::DoDataExchange(CDataExchange* pDX)
{
    ...
    DDX_DateTimeCtrl(pDX, IDC_DATETIMEPICKER_DATEMS, m_ctDateMS);
    DDX_DateTimeCtrl(pDX, IDC_DATETIMEPICKER_TIMEMS, m_ctTimeMS);
...
BEGIN_MESSAGE_MAP(CDlgUHFBasicInfo, CDialogEx)
    ...
    ON_BN_CLICKED(IDC_BUTTON_GET_TIMEMS, &CDlgUHFBasicInfo::OnBnClickedButtonGetTimems)
    ON_BN_CLICKED(IDC_BUTTON_SET_TIMEMS, &CDlgUHFBasicInfo::OnBnClickedButtonSetTimems)
END_MESSAGE_MAP()
...
void CDlgUHFBasicInfo::OnBnClickedButtonGetTimems()
{
    // TODO:  在此添加控件通知处理程序代码
    NET_DVR_CALIBRATE_TIME struTime = { 0 };
    struTime.dwSize = sizeof(struTime);

    DWORD dwReturn = 0;
    if (!NET_DVR_GetDVRConfig(m_lUserID, NET_DVR_GET_CALIBRATE_TIME, 0, &struTime, sizeof(NET_DVR_CALIBRATE_TIME), &dwReturn))
    {
        g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_FAIL_T, "NET_DVR_GET_CALIBRATE_TIME");
    }
    else
    {
        g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_SUCC_T, "NET_DVR_GET_CALIBRATE_TIME");
        m_ctDateMS.SetDate(struTime.struTime.dwYear, struTime.struTime.dwMonth, struTime.struTime.dwDay);
        m_ctTimeMS.SetTime(struTime.struTime.dwHour, struTime.struTime.dwMinute, struTime.struTime.dwSecond);
    }
    m_iMS = struTime.wMilliSec;
    UpdateData(FALSE);
}


void CDlgUHFBasicInfo::OnBnClickedButtonSetTimems()
{
    // TODO:  在此添加控件通知处理程序代码
    UpdateData(TRUE);
    if (m_iMS < 0 || m_iMS >= 1000)
    {
        CString csStr;
        csStr.Format("ms data error!\n");
        MessageBox(csStr);
    }

    NET_DVR_CALIBRATE_TIME struTime = { 0 };
    struTime.dwSize = sizeof(struTime);
    struTime.struTime.dwYear = m_ctDateMS.GetYear();
    struTime.struTime.dwMonth = m_ctDateMS.GetMonth();
    struTime.struTime.dwDay = m_ctDateMS.GetDay();
    struTime.struTime.dwHour = m_ctTimeMS.GetHour();
    struTime.struTime.dwMinute = m_ctTimeMS.GetMinute();
    struTime.struTime.dwSecond = m_ctTimeMS.GetSecond();
    struTime.wMilliSec = m_iMS;
    if (!NET_DVR_SetDVRConfig(m_lUserID, NET_DVR_SET_CALIBRATE_TIME, 0, &struTime, sizeof(NET_DVR_CALIBRATE_TIME)))
    {
        g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_FAIL_T, "NET_DVR_SET_CALIBRATE_TIME");
    }
    else
    {
        g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_SUCC_T, "NET_DVR_SET_CALIBRATE_TIME");
    }
}
