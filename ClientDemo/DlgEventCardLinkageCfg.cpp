...
void DlgEventCardLinkageCfg::SubEventTypeCardReader()
{
    ...
    g_StringLanType(szLan, "人脸采集失败", "Face Recognize Fail");
    ...
    g_StringLanType(szLan, "非正规M1卡认证失败", "Inforaml m1 card verify fail");
    m_comboSubEventType.InsertString(EVENT_ACS_INFORMAL_M1_CARD_VERIFY_FAIL, szLan);
    ...
    g_StringLanType(szLan, "虹膜认证通过", "IRIS verify passed");
    m_comboSubEventType.InsertString(EVENT_ACS_IRIS_VERIFY_PASS, szLan);
    g_StringLanType(szLan, "虹膜认证失败", "IRIS verify failed");
    m_comboSubEventType.InsertString(EVENT_ACS_IRIS_VERIFY_FAIL, szLan);
    g_StringLanType(szLan, "虹膜活体检测失败", "IRIS bioassay failed");
    m_comboSubEventType.InsertString(EVENT_ACS_IRIS_BIOASSAY_FAIL, szLan);
    g_StringLanType(szLan, "Felica卡识别未启用", "felica card recognize not enabled");
    m_comboSubEventType.InsertString(EVENT_ACS_FELICA_CARD_RECOGNIZE_NOT_ENABLED, szLan);
    g_StringLanType(szLan, "二维码识别成功", "QR code recognize passed");
    m_comboSubEventType.InsertString(EVENT_ACS_ORCODE_RECOGNIZE_PASS, szLan);
