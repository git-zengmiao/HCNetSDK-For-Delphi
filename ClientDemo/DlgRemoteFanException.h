...
class CDlgRemoteFanException : public CDialog
{
	DECLARE_DYNAMIC(CDlgRemoteFanException)

public:
	CDlgRemoteFanException(CWnd* pParent = NULL);   // standard constructor
	virtual ~CDlgRemoteFanException();
...
public:
	...
	//{{AFX_DATA(CDlgRemoteFanException)
	enum { IDD = IDD_DLG_REOMTE_TEMPEXCEPTION };
	...
	int m_iMin82;
	BOOL	m_bTempAlarm;
	float	m_fTempDownLimited;
	float	m_fTempUpLimited;
	//}}AFX_DATA
	
	//{{AFX_VIRTUAL(CDlgRemoteFanException)
protected:
	virtual void DoDataExchange(CDataExchange* pDX);    // DDX/DDV support
	//}}AFX_VIRTUAL
protected:
	// Generated message map functions
	//{{AFX_MSG(CDlgRemoteFanException)
