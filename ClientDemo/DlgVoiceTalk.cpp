...
void CALLBACK fAudioTalkDataCallBack(LONG lVoiceComHandle,char *pRecvDataBuffer,DWORD dwBufSize,BYTE byAudioFlag,void* pUser)
{
    ...
    if (byAudioFlag == 0)
    {
        printf("Local dwBufSize[%d]\n", dwBufSize);
    }
    else if (byAudioFlag == 1)
    {
        printf("Recv dwBufSize[%d]\n", dwBufSize);
    }
...
void CDlgVoiceTalk::OnBtnStart() 
{
    ...
    if (NET_DVR_GetLinkAddr(g_lVoiceHandle, ENUM_LINK_VOICEPLAY, &struParam))
    {
        g_pMainDlg->AddLog(m_dwDeviceIndex, OPERATION_SUCC_T, "NET_DVR_GetLinkAddr, dev addr[%s:%d], local addr[%s:%d]", \
            &(struParam.uDevIP), struParam.wDevPort[0], &(struParam.uLocalIP), struParam.wLocalPort[0]);
    }
    else
    {
        g_pMainDlg->AddLog(m_dwDeviceIndex, OPERATION_FAIL_T, "NET_DVR_GetLinkAddr");
    }
