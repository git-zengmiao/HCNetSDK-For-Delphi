...
void CDlgTsSeries::DoDataExchange(CDataExchange* pDX)
{
    ...
    DDX_Text(pDX, IDC_EDIT_VEHICLELOGORECOG, m_bVehicleLogoRecog);
}
...
void CDlgTsSeries::OnBtnGetstatus() 
{
    ...
    if (strParkingLampInfoCfg.byColorDepth == 0xff)
    {
        m_comCarColorDeep.SetCurSel(2);
    }
    else
    {
        m_comCarColorDeep.SetCurSel(strParkingLampInfoCfg.byColorDepth);
    }
    if (strParkingLampInfoCfg.byColor == 0xff)
    {
        m_comCarColor.SetCurSel(13);
    }
    else
    {
        m_comCarColor.SetCurSel(strParkingLampInfoCfg.byColor);
    }
    m_bVehicleLogoRecog = strParkingLampInfoCfg.byVehicleLogoRecog;
