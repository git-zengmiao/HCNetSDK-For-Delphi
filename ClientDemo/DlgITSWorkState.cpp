...
// 	}
	...
	for (i = 0; i < MAX_CHANNUM_V40; i++)  //
	{
        ...
        int iChanNum = g_struDeviceInfo[m_iDeviceIndex].pStruChanInfo[i].iChannelNO - 1;
        if (0 == m_struWorkState.struChanStatic[iChanNum].byRecordStatic)
		{
			g_StringLanType(szLan, "不录像", "Not Record");
			m_csRecState.Format(szLan);
		}
        else if (1 == m_struWorkState.struChanStatic[iChanNum].byRecordStatic)
		{
			g_StringLanType(szLan, "录像", "Record");
			m_csRecState.Format(szLan);
		}

        if (0 == m_struWorkState.struChanStatic[iChanNum].bySignalStatic)
		{
			g_StringLanType(szLan, "正常", "Normal");
			m_csSignalState.Format(szLan);
		}
        else if (1 == m_struWorkState.struChanStatic[iChanNum].bySignalStatic)
		{
			g_StringLanType(szLan, "信号丢失", "Signal loss");
			m_csSignalState.Format(szLan);
		}

        if (0 == m_struWorkState.struChanStatic[iChanNum].byHardwareStatic)
		{
			g_StringLanType(szLan, "正常", "Normal");
			m_csHardWareState.Format(szLan);
		}
        else if (1 == m_struWorkState.struChanStatic[iChanNum].byHardwareStatic)
		{
			g_StringLanType(szLan, "异常", "Abnormal");
			m_csHardWareState.Format(szLan);
		}

        m_csLinkCount.Format("%d", m_struWorkState.struChanStatic[iChanNum].dwLinkNum);
        iTotalLen += m_struWorkState.struChanStatic[iChanNum].dwLinkNum;
        m_csBitrate.Format("%ld", m_struWorkState.struChanStatic[iChanNum].dwBitRate);
		//strTemp.Format("%d", );
        ...
        m_listChanState.SetItemText(iIndex, 6, m_struWorkState.struChanStatic[iChanNum].struClientIP[0].sIpV4);
        strTemp.Format("%d", m_struWorkState.struChanStatic[iChanNum].dwIPLinkNum);
