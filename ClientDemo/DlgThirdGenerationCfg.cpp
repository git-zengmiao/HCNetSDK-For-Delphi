...
void CDlgThirdGenerationCfg::DoDataExchange(CDataExchange* pDX)
{
    ...
    DDX_Control(pDX, IDC_TREE_ALLOWLIST, m_TreeAlarm);
    DDX_Control(pDX, IDC_COMBO_ALLOWLIST, m_AllowListCtrl);
    ...
    DDX_Text(pDX, IDC_EDIT_ALLOWLIST, m_strAllowList);
...
BEGIN_MESSAGE_MAP(CDlgThirdGenerationCfg, CDialog)
    ...
    ON_CBN_SELCHANGE(IDC_COMBO_ALLOWLIST, OnSelchangeComboAllowlist)
    ON_NOTIFY(NM_CLICK, IDC_TREE_ALLOWLIST, OnClickTreeAllowlist)
	ON_WM_PAINT()
...
BOOL CDlgThirdGenerationCfg::OnInitDialog() 
{
    ...
    CString tmp;
    int i = 0;
    for (i = 0; i < MAX_ALLOWLIST_NUM; i++)
    {
        tmp.Format("Allow list %d", i + 1);
        m_AllowListCtrl.AddString(tmp);
    }
    m_AllowListCtrl.SetCurSel(0);
...
void CDlgThirdGenerationCfg::GetAlarmCount()
{
    m_iAlarmCount = 31;
    m_iPhoneCount = 4;
//     int sel = m_AllowListCtrl.GetCurSel();
//     for (int i = 0; i<32; i++)
//     {
//         if (0 != m_struSmsRelativeParam.struAllowList[sel].byAlarmHandler[i])
//         {
//             m_iAlarmCount++;
//         }       
//     }
...
//     for (i = 0; i<32; i++)
//     {
//         if (0 != m_struSmsRelativeParam.struAllowList[sel].byPhonePerssion[i])
//         {
//             m_iPhoneCount++;
//         }       
//     }
...
void CDlgThirdGenerationCfg::OnButtonGetsmsrelative() 
{
    ...
    m_strAllowList = m_struSmsRelativeParam.struAllowList[sel].byAllowList;
    m_csName = m_struSmsRelativeParam.struAllowList[sel].byName;
...
void CDlgThirdGenerationCfg::OnSelchangeComboAllowlist() 
{
	// TODO: Add your control notification handler code here
	UpdateData(TRUE);
    int sel = m_AllowListCtrl.GetCurSel();
...
void CDlgThirdGenerationCfg::OnClickTreeAllowlist(NMHDR* pNMHDR, LRESULT* pResult) 
{
    UpdateData(TRUE);
    int sel = m_AllowListCtrl.GetCurSel();
    CPoint pt(0,0);
    CRect rc(0,0,0,0);
    GetCursorPos(&pt);
    GetDlgItem(IDC_TREE_ALLOWLIST)->GetWindowRect(&rc);
    ScreenToClient(&rc);
    ScreenToClient(&pt);
    pt.x = pt.x - rc.left;
    pt.y = pt.y - rc.top;
    ...
    {
        m_struSmsRelativeParam.struAllowList[sel].byAlarmHandler[dwIndex] = !bCheck;
    }
    else
    {
        m_struSmsRelativeParam.struAllowList[sel].byPhonePerssion[dwIndex - m_iAlarmCount] = !bCheck;
    }
...
{	
    ...
    for (; i < m_iAlarmCount+m_iPhoneCount; i++)
    {
        ...
        if (i < m_iAlarmCount)
        { 
            if (m_struSmsRelativeParam.struAllowList[sel].byAlarmHandler[i] == 1)
            {
        ...
        {
            if (m_struSmsRelativeParam.struAllowList[sel].byPhonePerssion[i - m_iAlarmCount] == 1)
            {
                m_TreeAlarm.SetCheck(hItem, TRUE);
            }
        }
...
void CDlgThirdGenerationCfg::OnButtonSure() 
{
	// TODO: Add your control notification handler code here
    UpdateData(TRUE);
    int i = m_AllowListCtrl.GetCurSel();
	memcpy(m_struSmsRelativeParam.struAllowList[i].byAllowList, m_strAllowList, MAX_PHONE_NUM);
}
