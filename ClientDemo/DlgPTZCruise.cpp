	...
	, m_bPrivateIPMode(FALSE)
{
	...
	memset(&m_struCruisePointV50, 0, sizeof(m_struCruisePointV50));
}
...
// 		m_comboSeqSpeed.SetCurSel(m_struCruiseRet.struCruisePoint[m_iSeqPoint-1].Speed);
		...
		g_pMainDlg->AddLog(-1, OPERATION_SUCC_T, "NET_GET_CRUISEPOINT_V50 cruise[%d] point[%d]\
						   preset[%d] time[%d] speed[%d]", m_iCruiseNum, m_iSeqPoint, m_struCruisePointV50.struCruisePoint[m_iSeqPoint-1].wPresetNo,\
						   m_struCruisePointV50.struCruisePoint[m_iSeqPoint-1].wDwell, m_struCruisePointV50.struCruisePoint[m_iSeqPoint-1].bySpeed);
		m_comboSeqPreset.SetCurSel(m_struCruisePointV50.struCruisePoint[m_iSeqPoint-1].wPresetNo);
		m_comboSeqTime.SetCurSel(m_struCruisePointV50.struCruisePoint[m_iSeqPoint-1].wDwell);
				m_comboSeqSpeed.SetCurSel(m_struCruisePointV50.struCruisePoint[m_iSeqPoint-1].bySpeed);
	}
	else
	{
		//default
...
void CDlgPTZCruise::OnCbnSelchangeComboCruiseRoute()
{
	...
	*/
	...
	m_struCruisePointCond.dwChan = g_struDeviceInfo[iDeviceIndex].pStruChanInfo[iChanIndex].iChannelNO;



	m_struCruisePointCond.wRouteNo = m_iCruiseNum;
    BOOL bRet = NET_DVR_GetDeviceConfig(g_struDeviceInfo[iDeviceIndex].lLoginID, NET_GET_CRUISEPOINT_V50, 1, &m_struCruisePointCond, \
		sizeof(m_struCruisePointCond), &iStatus, &m_struCruisePointV50, sizeof(m_struCruisePointV50));
	if (bRet && iStatus == 0)
	{
		m_bPrivateIPMode = TRUE;
		g_pMainDlg->AddLog(iDeviceIndex, OPERATION_SUCC_T, "NET_GET_CRUISEPOINT_V50 cruise[%d] point[%d]preset[%d]dwell[%d]speed[%d] %s",\
			m_iCruiseNum, m_iSeqPoint,\
			m_struCruisePointV50.struCruisePoint[iSeqPoint].wPresetNo, m_struCruisePointV50.struCruisePoint[iSeqPoint].wDwell, \
			m_struCruisePointV50.struCruisePoint[iSeqPoint].bySpeed, \
			g_struDeviceInfo[iDeviceIndex].pStruChanInfo[iChanIndex].chChanName);
		m_comboSeqPreset.SetCurSel(m_struCruisePointV50.struCruisePoint[iSeqPoint].wPresetNo);
		m_comboSeqTime.SetCurSel(m_struCruisePointV50.struCruisePoint[iSeqPoint].wDwell);
		m_comboSeqSpeed.SetCurSel(m_struCruisePointV50.struCruisePoint[iSeqPoint].bySpeed);
		
    }
    else if (bRet && iStatus != 0)//接口返回成功，但是成功状态为非0，弹框显示失败的信息
    {
        m_bPrivateIPMode = FALSE;
        sprintf(szLan, "err %d", iStatus);
        AfxMessageBox(szLan);
        g_pMainDlg->AddLog(iDeviceIndex, OPERATION_FAIL_T, "NET_GET_CRUISEPOINT_V50");
    }
    else//接口返回失败，打印下日志即可
    {
        m_bPrivateIPMode = FALSE;
