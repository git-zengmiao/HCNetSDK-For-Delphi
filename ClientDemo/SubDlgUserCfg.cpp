...
/*********************************************************
  Function:	OnBnClickedBtnUserOkLinkToDvr
...
void CSubDlgUserCfg::OnBnClickedBtnUserOk()
{
    ...
    string strTemp = m_csVerifyPwd;
    reverse(strTemp.begin(), strTemp.end());
    if (strstr(strTemp.c_str(), m_csUserName) != NULL)
    {
        char szLan[128] = { 0 };
        g_StringLanType(szLan, "密码不能包含倒置用户名", "Password cannot contain an inverted user name");
        AfxMessageBox(szLan);
        return;
    }
