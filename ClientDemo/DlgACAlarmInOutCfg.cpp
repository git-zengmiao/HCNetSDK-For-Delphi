...
void CDlgACAlarmInOutCfg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_COMBO_FANGQU_TYPE, m_cmbFangquType);
...
{
    char szLan[128] = {0};
    m_cmbFangquType.ResetContent();
    int iIndex ; 
    g_StringLanType(szLan, "即时防区", "Instant zone");
    iIndex = m_cmbFangquType.AddString(szLan);
    m_cmbFangquType.SetItemData(iIndex, iIndex);
    ...
    iIndex = m_cmbFangquType.AddString(szLan);
    m_cmbFangquType.SetItemData(iIndex, 0xff);
}
...
void CDlgACAlarmInOutCfg::OnButAlarmInGet() 
{
    ...
    {
        m_cmbFangquType.SetCurSel(14);
    } 
    else
    {
        m_cmbFangquType.SetCurSel( struInParam.byType );
    }
...
void CDlgACAlarmInOutCfg::OnButAlarmInSet() 
{
    ...
    struInParam.byType = m_cmbFangquType.GetItemData(m_cmbFangquType.GetCurSel());
