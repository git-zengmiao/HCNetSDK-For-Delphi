...
CDlgAlarmNetUser::CDlgAlarmNetUser(CWnd* pParent /*=NULL*/)
    ...
    , m_bFireAlarm(FALSE)
    , m_bForceDeploy(FALSE)
    , m_bPermanentBypass(FALSE)
    , m_bAddCode(FALSE)
    , m_bDebugMode(FALSE)
{
...
void CDlgAlarmNetUser::DoDataExchange(CDataExchange* pDX)
{
    ...
    DDX_Text(pDX, IDC_EDIT_PASSWORD_KEYPAD, m_csKeypadPassword);
    DDV_MaxChars(pDX, m_csKeypadPassword, 16);
    DDX_Check(pDX, IDC_CHK_FIRE_ALARM, m_bFireAlarm);
    DDX_Check(pDX, IDC_CHK_FORCED_DEPLOY, m_bForceDeploy);
    DDX_Check(pDX, IDC_CHK_PERMANENT_BYPASS, m_bPermanentBypass);
    DDX_Check(pDX, IDC_CHK_ADD_CHANGE_PERSIONAL_CODE, m_bAddCode);
    DDX_Check(pDX, IDC_CHK_DEBUG_MODE, m_bDebugMode);
    DDX_Control(pDX, IDC_COMBO_USER_ENABLE, m_comboUserEnable);
}
...
BEGIN_MESSAGE_MAP(CDlgAlarmNetUser, CDialog)
    ...
    ON_WM_PAINT()
	//}}AFX_MSG_MAP
    ON_CBN_SELCHANGE(IDC_COMBO_USER_ENABLE, &CDlgAlarmNetUser::OnCbnSelchangeComboUserEnable)
END_MESSAGE_MAP()
...
//     }
    ...
    m_comboUserEnable.SetCurSel(m_struDeviceUser.byUserEnabled);
    OnCbnSelchangeComboUserEnable();
...
void CDlgAlarmNetUser::InitUserTypeCombo()
{
	...
	g_StringLanType(szLan, "普通用户(操作员)", "Normal user");
    m_comboUserType.AddString(szLan);
	g_StringLanType(szLan, "管理员", "administrator");
    m_comboUserType.AddString(szLan);
    g_StringLanType(szLan, "安装员用户", "installer user");
    m_comboUserType.AddString(szLan);
    g_StringLanType(szLan, "制造商用户", "manufacturer user");
    m_comboUserType.AddString(szLan);
}

void CDlgAlarmNetUser::InitUserEnableCombo()
{
    char szLan[128] = { 0 };
    m_comboUserEnable.ResetContent();
    g_StringLanType(szLan, "0-无效", "invalid");
    m_comboUserEnable.AddString(szLan);
    g_StringLanType(szLan, "1-开启", "open");
    m_comboUserEnable.AddString(szLan);
    g_StringLanType(szLan, "2-未开启", "close");
    m_comboUserEnable.AddString(szLan);
    UpdateData(FALSE);
}
...
void CDlgAlarmNetUser::GetDeviceUserFromWnd(NET_DVR_ALARM_DEVICE_USER &struDeviceUser)
{
    ...
    strncpy((char*)struDeviceUser.sKeypadPassword, m_csKeypadPassword.GetBuffer(0), sizeof(struDeviceUser.sKeypadPassword));
    struDeviceUser.byUserType = m_comboUserType.GetCurSel();
    struDeviceUser.byUserEnabled = m_comboUserEnable.GetCurSel();
...
//     struDeviceUser.byOtherRight[15] = m_bSensorCtrl;
    struDeviceUser.byOtherRight[11] = m_bFireAlarm;
    struDeviceUser.byOtherRight[12] = m_bForceDeploy;
    struDeviceUser.byOtherRight[13] = m_bPermanentBypass;
    struDeviceUser.byOtherRight[14] = m_bAddCode;
    struDeviceUser.byOtherRight[15] = m_bDebugMode;
...
void CDlgAlarmNetUser::SetDeviceUserToWnd(NET_DVR_ALARM_DEVICE_USER &struDeviceUser)
{
    ...
    memset(szLan, 0, sizeof(szLan));
    strncpy(szLan, (char*)struDeviceUser.sKeypadPassword, sizeof(struDeviceUser.sKeypadPassword));
    m_csKeypadPassword = szLan;

    m_comboUserType.SetCurSel(struDeviceUser.byUserType);

    m_comboUserEnable.SetCurSel(struDeviceUser.byUserEnabled);
	UpdateData(FALSE);
...
//    m_bSensorCtrl = struDeviceUser.byOtherRight[15];
   m_bFireAlarm = struDeviceUser.byOtherRight[11];
   m_bForceDeploy = struDeviceUser.byOtherRight[12];
   m_bPermanentBypass = struDeviceUser.byOtherRight[13];
   m_bAddCode = struDeviceUser.byOtherRight[14];
   m_bDebugMode = struDeviceUser.byOtherRight[15];
...
void CDlgAlarmNetUser::OnCbnSelchangeComboUserEnable()
{
    UpdateData(TRUE);
}
