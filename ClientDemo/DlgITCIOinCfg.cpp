...
void CDlgITCIOinCfg::AddVehInfotoDlg(int iItemCount, LPNET_DVR_VEHICLE_CONTROL_LIST_INFO lpInter)
{
    ...
    else if (VCA_CONSULATE_PLATE == lpInter->byPlateType)
    {
        g_StringLanType(szLan, "领事馆车牌", "consulate Plate");
        m_lstVehicle.SetItemText(iItemCount, 2, szLan);
    }
    else if (VCA_EMERGENCY_PLATE == lpInter->byPlateType)
    {
        g_StringLanType(szLan, "应急车牌", "emergency plate");
        m_lstVehicle.SetItemText(iItemCount, 2, szLan);
    }
    else if (TRANSPORT_PLATE == lpInter->byPlateType)
    {
        g_StringLanType(szLan, "运输车", "Transport");
        m_lstVehicle.SetItemText(iItemCount, 2, szLan);
    }
    else if (COMMERCIAL_PLATE == lpInter->byPlateType)
    {
        g_StringLanType(szLan, "商用车", "Commercial");
        m_lstVehicle.SetItemText(iItemCount, 2, szLan);
    }
    else if (PRIVATE_PLATE == lpInter->byPlateType)
    {
        g_StringLanType(szLan, "私家车", "Private car");
        m_lstVehicle.SetItemText(iItemCount, 2, szLan);
    }
    else if (LEARNING_PLATE == lpInter->byPlateType)
    {
        g_StringLanType(szLan, "教练车", "Learning");
        m_lstVehicle.SetItemText(iItemCount, 2, szLan);
    }
    else if (CD_PLATE == lpInter->byPlateType)
    {
        g_StringLanType(szLan, "使馆车(CD)", "Embassy car(CD)");
        m_lstVehicle.SetItemText(iItemCount, 2, szLan);
    }
    else if (CC_PLATE == lpInter->byPlateType)
    {
        g_StringLanType(szLan, "使馆车(CC)", "Embassy car(CC)");
        m_lstVehicle.SetItemText(iItemCount, 2, szLan);
    }
    else if (SPECIAL_PLATE == lpInter->byPlateType)
    {
        g_StringLanType(szLan, "特种车", "Special car");
        m_lstVehicle.SetItemText(iItemCount, 2, szLan);
    }
    else if (PROTOCOL_PLATE == lpInter->byPlateType)
    {
        g_StringLanType(szLan, "PROTOCOL", "PROTOCOL");
        m_lstVehicle.SetItemText(iItemCount, 2, szLan);
    }
    else if (GOVERNMENT_PLATE == lpInter->byPlateType)
    {
        g_StringLanType(szLan, "政府车", "Government Car");
        m_lstVehicle.SetItemText(iItemCount, 2, szLan);
    }
    else if (EXPORT_PLATE == lpInter->byPlateType)
    {
        g_StringLanType(szLan, "EXPORT", "EXPORT");
        m_lstVehicle.SetItemText(iItemCount, 2, szLan);
    }
    else if (TAXI_PLATE == lpInter->byPlateType)
    {
        g_StringLanType(szLan, "出租车", "Taxi");
        m_lstVehicle.SetItemText(iItemCount, 2, szLan);
    }
    else if (TESTING_PLATE == lpInter->byPlateType)
    {
        g_StringLanType(szLan, "TESTING", "TESTING");
        m_lstVehicle.SetItemText(iItemCount, 2, szLan);
    }
    else if (TRANSFER_PLATE == lpInter->byPlateType)
    {
        g_StringLanType(szLan, "TRANSFER", "TRANSFER");
        m_lstVehicle.SetItemText(iItemCount, 2, szLan);
    }
    else if (TRUCK_PLATE == lpInter->byPlateType)
    {
        g_StringLanType(szLan, "货车", "TRUCK");
        m_lstVehicle.SetItemText(iItemCount, 2, szLan);
    }
    else if (BUS_PLATE == lpInter->byPlateType)
    {
        g_StringLanType(szLan, "公交车", "BUS");
        m_lstVehicle.SetItemText(iItemCount, 2, szLan);
    }
    else if (PUBLIC_PLATE == lpInter->byPlateType)
    {
        g_StringLanType(szLan, "PUBLIC", "PUBLIC");
        m_lstVehicle.SetItemText(iItemCount, 2, szLan);
    }
    else if (PUB_TRANS_PLATE == lpInter->byPlateType)
    {
        g_StringLanType(szLan, "PUBLIC TRANSFER", "PUBLIC TRAN");
        m_lstVehicle.SetItemText(iItemCount, 2, szLan);
    }
    else if (PRI_TRANS_PLATE == lpInter->byPlateType)
    {
        g_StringLanType(szLan, "PRIVATE TRANSPORT", "PRIVATE TRANSPORT");
        m_lstVehicle.SetItemText(iItemCount, 2, szLan);
    }
    else if (TUMBREL_PLATE == lpInter->byPlateType)
    {
        g_StringLanType(szLan, "二轮车", "TUMBREL Plate");
        m_lstVehicle.SetItemText(iItemCount, 2, szLan);
    }
    else if (HONG_KONG_MACAO_PLATE == lpInter->byPlateType)
    {
    ...
    else if (VCA_BKAIR_PLATE == lpInter->byPlateColor)
    {
        g_StringLanType(szLan, "民航黑色车牌", "Civil aviation black license plate");
        m_lstVehicle.SetItemText(iItemCount, 3, szLan);
    }
    else if (VCA_RED_PLATE == lpInter->byPlateColor)
    {
        g_StringLanType(szLan, "红色车牌", "Red license plate");
        m_lstVehicle.SetItemText(iItemCount, 3, szLan);
    }
    else if (VCA_ORANGE_PLATE == lpInter->byPlateColor)
    {
        g_StringLanType(szLan, "橙色车牌", "Orange license plate");
        m_lstVehicle.SetItemText(iItemCount, 3, szLan);
    }
    else if (VCA_BROWN_PLATE == lpInter->byPlateColor)
    {
        g_StringLanType(szLan, "棕色车牌", "Brown license plate");
        m_lstVehicle.SetItemText(iItemCount, 3, szLan);
    }
