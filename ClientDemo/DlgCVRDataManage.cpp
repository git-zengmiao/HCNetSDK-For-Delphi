...
void CDlgCVRDataManage::OnBnClickedButtonBrowse()
{
    ...
    if (dlg.DoModal() == IDOK)
    {
        ...
        int iPos = m_strFilePath.ReverseFind('\\');
        if (iPos == -1)
        {
            return;
        }
        int n = m_strFilePath.GetLength();
        m_strFileName = m_strFilePath.Right(n - iPos - 1).GetBuffer(0);
        //m_strFileName = dlg.GetFileName();    //文件名长度超过64位时，这个方法会截断
        CFile cFile;
