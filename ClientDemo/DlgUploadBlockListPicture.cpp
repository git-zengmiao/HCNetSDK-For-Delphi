// DlgUploadBlockListPicture.cpp : 实现文件
//

#include "stdafx.h"
#include "ClientDemo.h"
#include "DlgUploadBlockListPicture.h"
#include "afxdialogex.h"

// 这个参数可从能力集获取，这里设置512K
#define MAX_PICTURE_LEN 1024 * 1024
#define WM_MSG_UPLOAD_PICTURE_FINISH 1002
// CDlgUploadBlockListPicture 对话框

IMPLEMENT_DYNAMIC(CDlgUploadBlockListPicture, CDialogEx)

CDlgUploadBlockListPicture::CDlgUploadBlockListPicture(CWnd* pParent /*=NULL*/)
    : CDialogEx(CDlgUploadBlockListPicture::IDD, pParent)
    , m_szName(_T(""))
    , m_szPictureName(_T(""))
    , m_dwPictureLen(0)
    , m_szCardNumber(_T(""))
...
CDlgUploadBlockListPicture::~CDlgUploadBlockListPicture()
{
}

void CDlgUploadBlockListPicture::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CDlgUploadBlockListPicture, CDialogEx)
    ON_BN_CLICKED(IDC_BUTTON_BROWSE, &CDlgUploadBlockListPicture::OnBnClickedButtonBrowse)
    ON_BN_CLICKED(IDC_BUTTON_ADD, &CDlgUploadBlockListPicture::OnBnClickedButtonAdd)
    ON_BN_CLICKED(IDC_BUTTON_CLEAR, &CDlgUploadBlockListPicture::OnBnClickedButtonClear)
    ON_BN_CLICKED(IDC_BUTTON_UPLOAD, &CDlgUploadBlockListPicture::OnBnClickedButtonUpload)
    ON_BN_CLICKED(IDOK, &CDlgUploadBlockListPicture::OnBnClickedOk)
    ON_MESSAGE(WM_MSG_UPLOAD_PICTURE_FINISH, OnMsgUploadPictureCfgFinish)
    ON_WM_DESTROY()
    ON_BN_CLICKED(IDC_BUTTON_DELETE, &CDlgUploadBlockListPicture::OnBnClickedButtonDelete)
    ON_BN_CLICKED(IDC_CHECK_ISDELETE, &CDlgUploadBlockListPicture::OnBnClickedCheckIsdelete)
END_MESSAGE_MAP()


// CDlgUploadBlockListPicture 消息处理程序


LRESULT CDlgUploadBlockListPicture::OnMsgUploadPictureCfgFinish(WPARAM wParam, LPARAM lParam)
{
    ...
    m_lUploadPicCfgHandle = -1;
    g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_SUCC_T, "NET_DVR_BULK_UPLOAD_BLOCK_LIST_PICTURE Upload finished");
    return 0;
}

void CALLBACK g_fUploadPictureCallback(DWORD dwType, void* lpBuffer, DWORD dwBufLen, void* pUserData)
{
    CDlgUploadBlockListPicture* pDlg = (CDlgUploadBlockListPicture*)pUserData;
    if (pDlg == NULL)
    {
        return;
    }
...
void CDlgUploadBlockListPicture::ProcessUploadPictureCfgCallbackData(DWORD dwType, void* lpBuffer, DWORD dwBufLen)
{
    ...
    {
        NET_DVR_BLOCKLIST_PICTURE_STATUS struCfg = { 0 };
...
void CDlgUploadBlockListPicture::OnBnClickedButtonBrowse()
{
    // TODO:  在此添加控件通知处理程序代码
...
BOOL CDlgUploadBlockListPicture::OnInitDialog()
{
    ...
    g_StringLanType(szLan, "禁止名单关联卡号", "Card No");
...
void CDlgUploadBlockListPicture::OnBnClickedButtonAdd()
{
    // TODO:  在此添加控件通知处理程序代码
    UpdateData(TRUE);

    //1.get data from control
    NET_DVR_BLOCKLIST_PICTURE_CFG struPictureCfg = { 0 };
    ...
    //2.is duplicated?
    LPNET_DVR_BLOCKLIST_PICTURE_CFG pPictureInfo = NULL;
    int iItemIndex = GetExistItem(&struPictureCfg);
    if (iItemIndex == -1)
    {
        pPictureInfo = new NET_DVR_BLOCKLIST_PICTURE_CFG;
        if (pPictureInfo == NULL)
        {
            return;
        }
        int iSize = sizeof(NET_DVR_BLOCKLIST_PICTURE_CFG);
    ...
    {
        pPictureInfo = (LPNET_DVR_BLOCKLIST_PICTURE_CFG)m_listPicture.GetItemData(iItemIndex);
    }
...
void CDlgUploadBlockListPicture::OnBnClickedButtonClear()
{
    // TODO:  在此添加控件通知处理程序代码
    UpdateData(TRUE);
...
void CDlgUploadBlockListPicture::OnBnClickedButtonUpload()
{
    // TODO:  在此添加控件通知处理程序代码
    ...
    NET_DVR_BLOCKLIST_PICTURE_COND struCond = { 0 };
    struCond.dwSize = sizeof(struCond);
    struCond.dwPictureNum = m_dwPictureNum;

    m_lUploadPicCfgHandle = NET_DVR_StartRemoteConfig(m_lServerID, NET_DVR_BULK_UPLOAD_BLOCKLIST_PICTURE, &struCond, sizeof(struCond), g_fUploadPictureCallback, this);
    if (m_lUploadPicCfgHandle == -1)
    {
        g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_FAIL_T, "NET_DVR_BULK_UPLOAD_BLOCK_LIST_PICTURE failed");
        return;
    }
    else
    {
        g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_SUCC_T, "NET_DVR_BULK_UPLOAD_BLOCK_LIST_PICTURE success");
    }
 ...
 */
BOOL CDlgUploadBlockListPicture::SendFirstPicture()
{
    int nItemCount = m_listPicture.GetItemCount();
    ...
    m_dwSendIndex = 0;
    //获取第一个禁止名单配置数据
    LPNET_DVR_BLOCKLIST_PICTURE_CFG pPictureInfo = (LPNET_DVR_BLOCKLIST_PICTURE_CFG)m_listPicture.GetItemData(m_dwSendIndex);
...
*  @return  HPR_TRUE--success, HPR_FALSE--fail.
*/
BOOL CDlgUploadBlockListPicture::SendNextPicture()
{
    int nItemCount = m_listPicture.GetItemCount();
...
/** @fn  CDlgUploadBlockListPicture::GetExistItem(const NET_DVR_BLOCK_LIST_PICTURE_CFG *lpCardCfg)
 *  @brief 从列表中查找是否存在相同的实例
 *  @param(IN) NET_DVR_BLOCK_LIST_PICTURE_CFG *lpCardCfg
 *  @return  index--the index, -1--no duplicate.
 */
int CDlgUploadBlockListPicture::GetExistItem(const NET_DVR_BLOCKLIST_PICTURE_CFG *lpCardCfg)
{
    int nItemCount = m_listPicture.GetItemCount();
    int i = 0;
    LPNET_DVR_BLOCKLIST_PICTURE_CFG lpTemp = NULL;
    for (i = 0; i < nItemCount; i++)
    {
        lpTemp = (LPNET_DVR_BLOCKLIST_PICTURE_CFG)m_listPicture.GetItemData(i);
        if (!lpCardCfg)
        {
            continue;
        }
...
/** @fn  char * CDlgUploadBlockListPicture::ReadPictureFromFile(CString fileName)
 *  @brief 
 *  @param(IN) 
 *  @return  char * -- success, NULL --fail.
 */
char * CDlgUploadBlockListPicture::ReadPictureFromFile(CString fileName)
{
    //
 ...
 *  @return  void
 */
void CDlgUploadBlockListPicture::Clear()
{
    int nItemCount = m_listPicture.GetItemCount();
    for (int i = 0; i < nItemCount; i++)
    {
        LPNET_DVR_BLOCKLIST_PICTURE_CFG pPictureInfo = (LPNET_DVR_BLOCKLIST_PICTURE_CFG)m_listPicture.GetItemData(i);
...
void CDlgUploadBlockListPicture::OnBnClickedOk()
{
    // TODO:  在此添加控件通知处理程序代码
    Clear();
    CDialogEx::OnOK();
...
void CDlgUploadBlockListPicture::OnDestroy()
{
    CDialogEx::OnDestroy();
    Clear();
    // TODO:  在此处添加消息处理程序代码
...
void CDlgUploadBlockListPicture::OnBnClickedButtonDelete()
{
    // TODO:  在此添加控件通知处理程序代码
    CDlgUploadBlockListPicture::OnBnClickedButtonUpload();
}


void CDlgUploadBlockListPicture::OnBnClickedCheckIsdelete()
{
    // TODO:  在此添加控件通知处理程序代码
    UpdateData(TRUE);
    if (m_bIsValid)
