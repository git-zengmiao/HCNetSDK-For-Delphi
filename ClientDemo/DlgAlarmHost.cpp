...
void CDlgAlarmHost::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_COMBO_FANGQU_TYPE, m_comboFangquType);
...
BEGIN_MESSAGE_MAP(CDlgAlarmHost, CDialog)
	...
	ON_CBN_SELCHANGE(IDC_COMBO_FANGQU_TYPE, OnSelchangeComboFangquType)
...
void CDlgAlarmHost::InitAlarmInDetectorCombo()
{
    ...
    g_StringLanType(szLan, "水浸探测器", "smart lock");
    m_comboDetector.InsertString(nIndex, szLan);
...
void CDlgAlarmHost::IninAlarmInFangquTypeCombo()
{
    char szLan[128] = {0};
    m_comboFangquType.ResetContent();
    g_StringLanType(szLan, "即时防区", "Instant zone");
    m_comboFangquType.AddString(szLan);
...
void CDlgAlarmHost::SetAlarmInParamToWnd(NET_DVR_ALARMIN_PARAM& struAlarmInParam)
{
	...
	{
		m_comboFangquType.SetCurSel(m_comboFangquType.GetCount()-1);
	}
	else
	{
		m_comboFangquType.SetCurSel(struAlarmInParam.byType);
	}
    m_comboFangquType.SetCurSel(struAlarmInParam.byType);
    OnSelchangeComboFangquType();
...
void CDlgAlarmHost::GetAlarmInParamFromWnd(NET_DVR_ALARMIN_PARAM& struAlarmInParam)
{
    ...
    struAlarmInParam.byType = (m_comboFangquType.GetCurSel()>13)?0xff:m_comboFangquType.GetCurSel();
...
void CDlgAlarmHost::OnSelchangeComboFangquType() 
{
    if (m_comboFangquType.GetCurSel() == 2)
    {
