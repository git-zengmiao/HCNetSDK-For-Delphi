...
void CDlgAlarmhostSubsystem::DoDataExchange(CDataExchange* pDX)
{
    ...
    DDX_Control(pDX, IDC_LIST_JOINT_NET_USER, m_listAssociateNetUser);
}
...
void CDlgAlarmhostSubsystem::InitListCtrl() 
{
    ...
    //关联网络用户
    m_listAssociateNetUser.SetExtendedStyle(LVS_EX_CHECKBOXES);
    for (i = 0; i < m_struAlarmHostAbility.wNetUserNum; i++)
    {
        sprintf(szLan, "Net User %d", (i + 1));
        m_listAssociateNetUser.InsertItem(i, szLan);
    }
...
void CDlgAlarmhostSubsystem::OnBtnGet() 
{
    ...
    for (i = 0; i < m_listAssociateNetUser.GetItemCount(); i++)
    {
        m_listAssociateNetUser.SetCheck(i, (BOOL)((m_struSubSystemEx.byJointNetUser[i/8]>>(i%8))&0x01));
    }
...
// 	}
    ...
    for (i = 0; i < m_listAssociateNetUser.GetItemCount(); i++)
    {
        m_struSubSystemEx.byJointNetUser[i/8] |= (BOOL)(m_listAssociateNetUser.GetCheck(i)<<(i%8));
    }
