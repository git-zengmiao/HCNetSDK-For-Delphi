	...
	: CDialog(CDlgITSMiniPost::IDD, pParent)
{
	...
	m_bChkPlateType_CivilAndSpecial = FALSE;
...
void CDlgITSMiniPost::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Check(pDX, IDC_CHECK_PLATETYPE_CIVIL_AND_SPECIAL, m_bChkPlateType_CivilAndSpecial);
...
void CDlgITSMiniPost::AddTrafficDataItem(const NET_DVR_TRAFFIC_DATA_QUERY_RESULT &Item)
{
    ...
    if (bTemp)
    {
        sprintf(szStr, "%s标准民用车与特种车;", szStr);
    }
...
void CDlgITSMiniPost::SetTrafficDataCond()
{
    ...
    m_struTrafficDataQueryCond.dwPlateType |= (m_bChkPlateType_CivilAndSpecial << 1);
