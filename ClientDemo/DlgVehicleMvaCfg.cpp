	...
	: CDialog(CDlgVehicleMvaCfg::IDD, pParent)
{
	...
	m_bBlockListAlarm = FALSE;
	m_bBlockListCheck = FALSE;
	m_bBlockListHint = FALSE;
	m_bBlockListUpload = FALSE;
...
void CDlgVehicleMvaCfg::DoDataExchange(CDataExchange* pDX)
{
    ...
    DDX_Check(pDX, IDC_CHK_BLOCKLIST_ALARM, m_bBlockListAlarm);
    DDX_Check(pDX, IDC_CHK_BLOCKLIST_CHECK, m_bBlockListCheck);
    DDX_Check(pDX, IDC_CHK_BLOCKLIST_HINT, m_bBlockListHint);
    DDX_Check(pDX, IDC_CHK_BLOCKLIST_UPLOAD, m_bBlockListUpload);
...
BEGIN_MESSAGE_MAP(CDlgVehicleMvaCfg, CDialog)
	...
	ON_BN_CLICKED(IDC_BTN_SET_BLOCKLIST_CFG, OnBtnSetBlockListCfg)
	ON_BN_CLICKED(IDC_BTN_GET_BLOCKLIST_CFG, OnBtnGetBlockListCfg)
...
void CDlgVehicleMvaCfg::OnBtnSetBlockListCfg() 
{
	UpdateData(TRUE);
	memset(&m_struBlockListCfg, 0, sizeof(NET_DVR_MOBILE_LOCALPLATECHK_CFG));
	m_struBlockListCfg.dwSize = sizeof(NET_DVR_MOBILE_LOCALPLATECHK_CFG);
	m_struBlockListCfg.byCheck = m_bBlockListCheck;
	m_struBlockListCfg.byCheckAlarm = m_bBlockListAlarm;
	m_struBlockListCfg.byCheckHint = m_bBlockListHint;
	m_struBlockListCfg.byUploadUnlicensedCar = m_bBlockListUpload;
	
	if (NET_DVR_SetDVRConfig(m_lServerID, NET_DVR_SET_MOBILE_LOCALPLATECHK_CFG, m_lChannal, &m_struBlockListCfg, sizeof(NET_DVR_MOBILE_LOCALPLATECHK_CFG)))
    {
...
void CDlgVehicleMvaCfg::OnBtnGetBlockListCfg() 
{
	DWORD dwReturn = 0;
	memset(&m_struBlockListCfg, 0, sizeof(NET_DVR_MOBILE_LOCALPLATECHK_CFG));
    if (NET_DVR_GetDVRConfig(m_lServerID, NET_DVR_GET_MOBILE_LOCALPLATECHK_CFG, m_lChannal, &m_struBlockListCfg, sizeof(NET_DVR_MOBILE_LOCALPLATECHK_CFG), &dwReturn))
    {
        g_pMainDlg->AddLog(m_dwDevIndex, OPERATION_SUCC_T, "NET_DVR_GET_MOBILE_LOCALPLATECHK_CFG");
		
		m_bBlockListCheck = m_struBlockListCfg.byCheck;
		m_bBlockListAlarm = m_struBlockListCfg.byCheckAlarm;
		m_bBlockListHint = m_struBlockListCfg.byCheckHint;
		m_bBlockListUpload = m_struBlockListCfg.byUploadUnlicensedCar;
    }
    else
    {
...
void CALLBACK g_fTransBlockListDataCallBack(DWORD dwType, void *pRecvDataBuffer, DWORD dwBufSize, void* pUserData)
{
	...
	case NET_SDK_CALLBACK_STATUS_FAILED:
		g_pMainDlg->AddLog(pThis->m_dwDevIndex, OPERATION_FAIL_T, "Send Block list info");
...
void CDlgVehicleMvaCfg::OnBtnVehicleDataSend() 
{
	...
	m_lTranHandle = NET_DVR_StartRemoteConfig(m_lServerID, NET_DVR_VEHICLE_CHECK_START, NULL, 0, g_fTransBlockListDataCallBack, this);
	if (m_lTranHandle  == -1)
	{
