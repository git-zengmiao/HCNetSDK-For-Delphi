...
BOOL CDlgVqdPlan::OnInitDialog() 
{
	...
	g_StringLanType(szLan, "布防点ID", "Monitor ID.");
...
void CDlgVqdPlan::OnBtnAdd() 
{
	...
	if (iItemCount > 64)
	{
		g_StringLanType(szLan, "布防计划超过64个", "Monitor Plan is more than 64");
		AfxMessageBox(szLan);
		return;
	}
...
void CDlgVqdPlan::OnBtnSet() 
{
	...
	if (bRet)
	{
		g_StringLanType(szLan,"设置布防点计划成功","Success");
		g_pMainDlg->AddLog(m_iDevIndex, OPERATION_SUCC_T, "Set Monitor Plan Success");
		AfxMessageBox(szLan);
	}
	else
	{        
		g_StringLanType(szLan,"设置布防点计划失败","Fail to set Monitor Plan");
...
void CDlgVqdPlan::OnBtnMoitorSet()
{
	...
	{        
		g_StringLanType(szLan,"设置布防点信息失败","Fail to set Monitor Plan");
		g_pMainDlg->AddLog(m_iDevIndex, OPERATION_FAIL_T, "Fail to set Monitor Plan");
		AfxMessageBox(szLan);
	}
...
// 	}
	...
	if (bRet)
	{
		g_StringLanType(szLan,"删除布防点计划成功","Success");
		g_pMainDlg->AddLog(m_iDevIndex, OPERATION_SUCC_T, "Set Monitor Plan Success");
		AfxMessageBox(szLan);
		m_lstVqdPlan.DeleteAllItems();
		m_lstMonitorInfo.DeleteAllItems();
	...
	{        
		g_StringLanType(szLan,"删除布防点计划失败","Fail to set Monitor Plan");
		g_pMainDlg->AddLog(m_iDevIndex, OPERATION_FAIL_T, "Fail to set Monitor Plan");
		AfxMessageBox(szLan);
	}
