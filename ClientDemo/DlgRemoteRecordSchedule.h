...
class CDlgRemoteRecordSchedule : public CDialog
{
	DECLARE_DYNAMIC(CDlgRemoteRecordSchedule)

public:
	CDlgRemoteRecordSchedule(CWnd* pParent = NULL);   // standard constructor
	virtual ~CDlgRemoteRecordSchedule();
...
public:
	...
	//{{AFX_DATA(CDlgRemoteRecordSchedule)
	enum { IDD = IDD_DLG_REMOTE_RECORD_SCHEDULE };
	...
	//{{AFX_VIRTUAL(CDlgRemoteRecordSchedule)
protected:
	virtual void DoDataExchange(CDataExchange* pDX);    // DDX/DDV support
	//}}AFX_VIRTUAL
protected:
	// Generated message map functions
	//{{AFX_MSG(CDlgRemoteRecordSchedule)
...
public:
    ...
    LONG    m_lChannel;     
    int     m_iDevIndex;
	NET_DVR_HOLIDAY_RECORD m_struHolidayRecord;
    BOOL GetHolidayRecord(NET_DVR_HOLIDAY_RECORD &struHolidayRecord);
    BOOL SetHolidayRecord(NET_DVR_HOLIDAY_RECORD &struHolidayRecord);
};
