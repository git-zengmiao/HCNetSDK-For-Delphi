...
#endif
{
	...
	if (nIDEvent == UPLOAD_TIMER)
    {
        ...
        {
            g_StringLanType(szLan, "文件名过长", "File name too long");
            m_csUploadState.Format(szLan);
        }
        else if (state == 62)
        {
            g_StringLanType(szLan, "文件名属于内置文件名", "Built in file name");
            m_csUploadState.Format(szLan);
        }
        else if (state == 63)
        {
            g_StringLanType(szLan, "播放器文件超限", "Video player file size exceeds limit");
            m_csUploadState.Format(szLan);
        }
