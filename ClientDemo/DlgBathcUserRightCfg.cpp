...
BOOL CDlgBathcUserRightCfg::OnInitDialog() 
{
	...
	g_StringLanType(szLan, "添加布防点权限", "Add Monitor Point Right");
	m_lstUserRight.InsertColumn(iIndex, szLan, LVCFMT_LEFT, 100, -1);
	iIndex++;
	
	g_StringLanType(szLan, "修改布防点权限", "Mod Monitor Point Right");
	m_lstUserRight.InsertColumn(iIndex, szLan, LVCFMT_LEFT, 100, -1);
	iIndex++;
	
	g_StringLanType(szLan, "删除布防点权限", "Del Monitor Point Right");
