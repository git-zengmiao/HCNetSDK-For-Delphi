...
void CDlgStreamMediaCfg::DoDataExchange(CDataExchange* pDX)
{
    ...
    DDX_Text(pDX, IDC_EDIT_DOMAIN_ID, m_nDomainID);
}
...
BOOL CDlgStreamMediaCfg::OnInitDialog()
{
    ...
    g_StringLanType(szLan, "域ID", "domain ID");
    m_listCtrlStreamMediaCfg.InsertColumn(7, szLan, LVCFMT_LEFT, 80, -1);
...
void CDlgStreamMediaCfg::OnBnClickedButtonSet()
{
    ...
    if (0 == iItemCount)
    {
		...
		strncpy((char*)pStreamMediaCfg->sUrl, m_csUrl, STREAM_ID_LEN);

        strncpy(pStreamMediaCfg->struDMSIP.sIpV4, m_dmsIP, 16);
        pStreamMediaCfg->wDMSPort = m_dmsPort;
        pStreamMediaCfg->dwDomainID = m_nDomainID;
    ...
    else
    {
        ...
        for (i = 0; i < iItemCount; i++)
        {
            ...
            strncpy((char*)pTempStreamMediaCfg->sUrl, m_listCtrlStreamMediaCfg.GetItemText(i, 3), STREAM_ID_LEN);
            ...
            pStreamMediaCfg->dwDomainID = _ttoi(m_listCtrlStreamMediaCfg.GetItemText(i, 7));
...
void CDlgStreamMediaCfg::OnBnClickedButtonGet()
{
    ...
    else
    {
        ...
        for (i = 0; i < iItemCount; i++)
        {
            ...
            CString strDomainID;
            strDomainID.Format("%d", pTempStreamMediaCfg->dwDomainID);
            m_listCtrlStreamMediaCfg.SetItemText(i, 7, strDomainID);
...
void CDlgStreamMediaCfg::OnClickListStreamMediaCfg(NMHDR* pNMHDR, LRESULT* pResult)
{
    ...
    else
    {
		...
		CString strDomainIDBuf = m_listCtrlStreamMediaCfg.GetItemText(nItem, 7);
        m_nDomainID = _ttoi(strDomainIDBuf);
