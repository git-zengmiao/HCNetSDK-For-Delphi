...
void CDlgUploadFaceLib::DoDataExchange(CDataExchange* pDX)
{
    ...
    DDX_Text(pDX, IDC_EDT_LibID, m_csLibID);
    DDX_Text(pDX, IDC_EDT_TEXT_PATH, m_csTextPath);
}
...
void CDlgUploadFaceLib::readFileData()
{
    ...
    DWORD dwFileSize = 0;
    //读Text文件
    strcpy(szFileName, m_csTextPath);
    CFile cFile;
...
DWORD WINAPI CDlgUploadFaceLib::DataLibUpLoadThread(LPVOID pParam)
{
...
void CDlgUploadFaceLib::OnBnClickedBtnUpload()
{
    ...
    if (m_comDataType.GetCurSel() == 0)
    {
        NET_DVR_FACELIB_COND struFaceLibCond = { 0 };
        struFaceLibCond.dwSize = sizeof(NET_DVR_FACELIB_COND);
        memcpy(struFaceLibCond.szFDID, m_csLibID.GetBuffer(0), m_csLibID.GetLength());
    ...
    if (m_comDataType.GetCurSel() == 1)
    {
        NET_DVR_HBDLIB_COND struHBDLib = { 0 };
        struHBDLib.dwSize = sizeof(NET_DVR_HBDLIB_COND);
        memcpy(struHBDLib.szHBDID, m_csLibID.GetBuffer(0), m_csLibID.GetLength());
        struHBDLib.byConcurrent = m_bConcurrent;
        struHBDLib.byCover = m_bCover;
        struHBDLib.byCustomHBDID = m_bCustomID;

        m_lUploadHandle = NET_DVR_UploadFile_V40(m_lUserID, IMPORT_DATA_TO_HBDLIB, &struHBDLib, sizeof(NET_DVR_HBDLIB_COND), NULL, NULL, 0);

        if (m_lUploadHandle < 0)
        {
            g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_FAIL_T, "IMPORT_DATA_TO_HBDLIB");
            return;
        }
        else
        {
            g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_SUCC_T, "IMPORT_DATA_TO_HBDLIB");
        }
    }
    ...
    DWORD dwThreadID = 0;
    m_hUpLoadThread = CreateThread(NULL, 0, LPTHREAD_START_ROUTINE(DataLibUpLoadThread), this, 0, &dwThreadID);
