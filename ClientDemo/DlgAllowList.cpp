// DlgAllowList.cpp : implementation file
//

#include "stdafx.h"
#include "clientdemo.h"
#include "DlgAllowList.h"
...
DlgAllowList::DlgAllowList(CWnd* pParent /*=NULL*/)
	: CDialog(DlgAllowList::IDD, pParent)
    , m_bChkDetectorConnectionReport(FALSE)
    , m_bChkDetectorPowerReport(FALSE)
    , m_bChkVideoAlarm(FALSE)
{
	//{{AFX_DATA_INIT(DlgAllowList)
...
void DlgAllowList::DoDataExchange(CDataExchange* pDX)
{
    CDialog::DoDataExchange(pDX);
    //{{AFX_DATA_MAP(DlgAllowList)
...
BEGIN_MESSAGE_MAP(DlgAllowList, CDialog)
	//{{AFX_MSG_MAP(DlgAllowList)
...
/////////////////////////////////////////////////////////////////////////////
// DlgAllowList message handlers


BOOL DlgAllowList::OnInitDialog()
{
...
{ 
	...
	//	m_struAllowlistAlarm.struAllowList[sel].byAlarmOnRight = m_comAlarmType.GetCurSel();
	...
	if (!NET_DVR_SetDVRConfig(m_lUserID, NET_DVR_SET_ALLOWLIST_ALARM, iSel, &struAlarmPhoneCfg, sizeof(struAlarmPhoneCfg)))
	{
		g_StringLanType(szLan, "设置允许名单配置失败" , "NET_DVR_SET_ALLOWLIST_ALARM FAILED");
		MessageBox(szLan);
		g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_FAIL_T, "NET_DVR_SET_ALLOWLIST_ALARM FAILED");
		return;
	}
	else
	{
		g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_SUCC_T, "NET_DVR_SET_ALLOWLIST_ALARM SUCC");
	}
}

void DlgAllowList::OnBtnGet() 
{
	// TODO: Add your control notification handler code here
	int iSel = m_comAlarmPhoneCfg.GetCurSel() + 1;
	...
	DWORD returned = 0;
	char szLan[128] = {0};
	if (!NET_DVR_GetDVRConfig(m_lUserID, NET_DVR_GET_ALLOWLIST_ALARM, iSel, &struAlarmPhoneCfg, sizeof(struAlarmPhoneCfg), &returned))
	{
		g_StringLanType(szLan, "获取允许名单配置失败" , "NET_DVR_GET_ALLOWLIST_ALARM FAILED");
		MessageBox(szLan);
		g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_FAIL_T, "NET_DVR_GET_ALLOWLIST_ALARM FAILED");
		return;
	}
	else
	{
		g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_SUCC_T, "NET_DVR_GET_ALLOWLIST_ALARM SUCC");
	}
	...
	//m_sPhone.Format("%s",m_struAllowlistAlarm.struAllowList[sel].byAllowList,32);
	char sPhone[33] = {0};
	strncpy(sPhone, (char*)struAlarmPhoneCfg.byPhoneNumber, MAX_PHONE_NUM);
	m_sPhone = 	sPhone;
	
	//m_comAlarmType.SetCurSel((int)m_struAllowlistAlarm.struAllowList[sel].byAlarmOnRight);
	//m_bClearAlarmPermission = (m_struAllowlistAlarm.struAllowList[sel].byAlarmOnRight >> 2)&0x01;
	int i = 0;
	for (i=0; i<m_listZoneReport.GetItemCount(); i++)
	{
...
void DlgAllowList::OnSelchangeCombIntervalTime() 
{
	// TODO: Add your control notification handler code here
	UpdateData(TRUE);
...
void DlgAllowList::OnSelchangeComAlarmPhoneCfg() 
{
// 	// TODO: Add your control notification handler code here
// 	UpdateData(TRUE);
// 	int sel = m_comAlarmPhoneCfg.GetCurSel();
// 	m_bEnable = m_struAllowlistAlarm.struAllowList[sel].byEnable;
// 	//m_sPhone.Format("%s",m_struAllowlistAlarm.struAllowList[sel].byAllowList,32);
// 	char sPhone[33] = {0};
// 	strncpy(sPhone, (char*)m_struAllowlistAlarm.struAllowList[sel].byAllowList, MAX_PHONE_NUM);
// 	m_sPhone = 	sPhone;
// 
// 	//m_comAlarmType.SetCurSel((int)m_struAllowlistAlarm.struAllowList[sel].byAlarmOnRight);
// 	//m_bClearAlarmPermission = (m_struAllowlistAlarm.struAllowList[sel].byAlarmOnRight >> 2)&0x01;
...
// 	{
// 		m_listZoneReport.SetCheck(i, (BOOL)m_struAllowlistAlarm.struAllowList[sel].byZoneReport[i]);
// 	}
// 	for (i=0; i<m_listArm.GetItemCount(); i++)
// 	{
// 		m_listArm.SetCheck(i, (BOOL)((m_struAllowlistAlarm.struAllowList[sel].dwArmRight >> i)&0x01));
// 	}
// 	for (i=0; i<m_listDisArm.GetItemCount(); i++)
// 	{
// 		m_listDisArm.SetCheck(i, (BOOL)((m_struAllowlistAlarm.struAllowList[sel].dwDisArmRight >> i)&0x01));
// 	}
// 	for (i=0; i<m_listClearAlarm.GetItemCount(); i++)
// 	{
// 		m_listClearAlarm.SetCheck(i, (BOOL)((m_struAllowlistAlarm.struAllowList[sel].dwClearAlarmRight >> i)&0x01));
// 	}
// 	m_bSoftZone = (m_struAllowlistAlarm.struAllowList[sel].dwNonZoneReport >> 0)&0x01;
// 	m_bSystemState = (m_struAllowlistAlarm.struAllowList[sel].dwNonZoneReport >> 1)&0x01;
// 	m_bCancelReport = (m_struAllowlistAlarm.struAllowList[sel].dwNonZoneReport >> 2)&0x01;
// 	m_bTest = (m_struAllowlistAlarm.struAllowList[sel].dwNonZoneReport >> 3)&0x01;
// 	m_bArm = (m_struAllowlistAlarm.struAllowList[sel].dwNonZoneReport >> 4)&0x01;
// 	m_bDisarm = (m_struAllowlistAlarm.struAllowList[sel].dwNonZoneReport >> 5)&0x01;
// 	m_bHjack = (m_struAllowlistAlarm.struAllowList[sel].dwNonZoneReport >> 6)&0x01;
// 	m_bAlarmRestore = (m_struAllowlistAlarm.struAllowList[sel].dwNonZoneReport >> 7)&0x01;
// 	m_bBypass = (m_struAllowlistAlarm.struAllowList[sel].dwNonZoneReport >> 8)&0x01;
// 	m_bBypassRestore = (m_struAllowlistAlarm.struAllowList[sel].dwNonZoneReport >> 9)&0x01;
// 	UpdateData(FALSE);
}
