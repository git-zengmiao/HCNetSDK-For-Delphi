...
DWORD WINAPI CDlgMergeDevPic::PicUpLoadByBufLongThread(LPVOID pParam)
{
    ...
    while (bKeepLoop)
    {
        ...
        case 3:
            ...
            if (pThis->m_lUploadByBufLongHandle >= 0)
            {
                NET_DVR_UploadClose(pThis->m_lUploadByBufLongHandle);
                pThis->m_lUploadByBufLongHandle = -1;
            }

            g_StringLanType(szLan, "上传失败，连接已断开", "link close!");
            AfxMessageBox(szLan);
            break;
        case 4:
            csStr = _T("网络断开，状态未知");
            ...
            g_StringLanType(szLan, "超时，连接已断开", "link close!");
            AfxMessageBox(szLan);
            break;
        default:
            pThis->m_bIsSending = FALSE;
