...
CDlgCardReaderCfg::CDlgCardReaderCfg(CWnd* pParent /*=NULL*/)
    ...
    , m_dwFpNum(0)
    , m_bIsSupport(FALSE)
    , m_dwBlockListThreshold(0)
    , m_bReverseCardNo(FALSE)
    , m_dwIndependSwipeIntervals(0)
{
...
void CDlgCardReaderCfg::DoDataExchange(CDataExchange* pDX)
{
    ...
    DDX_Control(pDX, IDC_CMB_DEFAULT_VERIFY_MODE, m_cmbDefaultVerifyMode);
    DDX_Control(pDX, IDC_CMB_SUPPORT_DEL_FP_BY_ID, m_cmbSupportDelFPByID);
    ...
    DDX_Text(pDX, IDC_EDIT_BLACK_FACE_CONTRAST_THRESHOLD, m_dwBlockListThreshold);
    DDX_Check(pDX, IDC_CHK_AC_REVERSE_ENABLE, m_bReverseCardNo);
    DDX_Text(pDX, IDC_EDIT_SWIPE_INTERVAL_INDEPEND, m_dwIndependSwipeIntervals);
}
...
void CDlgCardReaderCfg::OnButGet() 
{
    ...
    //4代表发送的为0xff
    if (struReaderCfg.byFaceContrastMotionDetLevel != 0xff)
    {
        m_cmbFaceContrastMotionDetLevel.SetCurSel(struReaderCfg.byFaceContrastMotionDetLevel);
    }
    else
    {
        m_cmbFaceContrastMotionDetLevel.SetCurSel(4);
    }

    m_cmbDefaultVerifyMode.SetCurSel(struReaderCfg.byDefaultVerifyMode);
    m_cmbSupportDelFPByID.SetCurSel(struReaderCfg.bySupportDelFPByID);
    ...
    m_dwBlockListThreshold = struReaderCfg.byBlockListMatchThreshold;
    m_dwIndependSwipeIntervals = struReaderCfg.dwIndependSwipeIntervals;
    m_bReverseCardNo = struReaderCfg.byEnableReverseCardNo;
...
void CDlgCardReaderCfg::OnButSet() 
{
    ...
    //4代表接收的为0xff
    if (m_cmbFaceContrastMotionDetLevel.GetCurSel() == 4)
    {
        struReaderCfg.byFaceContrastMotionDetLevel = 0xff;
    }
    else
    {
    ...
    struReaderCfg.byBlockListMatchThreshold = (BYTE)m_dwBlockListThreshold;
    struReaderCfg.dwIndependSwipeIntervals = m_dwIndependSwipeIntervals;
    struReaderCfg.byEnableReverseCardNo = (BYTE)m_bReverseCardNo;
...
BOOL CDlgCardReaderCfg::OnInitDialog() 
{
    ...
    m_cmbDefaultVerifyMode.SetCurSel(0);
    m_cmbSupportDelFPByID.SetCurSel(0);
