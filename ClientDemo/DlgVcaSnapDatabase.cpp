...
BOOL CDlgVcaSnapDatabase::OnInitDialog()
{
	...
	g_StringLanType(szLan, "¼ÇÂ¼ID", "Record ID");
...
void CDlgVcaSnapDatabase::OnBtnInquireSnapdb() 
{
	...
	if (m_lHandle < 0)
	{
        ...
        g_StringLanType(szLan, "²éÑ¯×¥ÅÄÊý¾Ý¿âÊ§°Ü", "Fail to inquire record");
        AfxMessageBox(szLan);
		return;
    }
    else
...
//		g_StringLanType(szLan, "²éÑ¯×¥ÅÄÊý¾Ý¿â³É¹¦", "Succ to inquire record");
//        AfxMessageBox(szLan);
    }
}
...
void CDlgVcaSnapDatabase::OnBtnStopInquireSnapdb() 
{
	...
	if (m_lHandle >= 0)
	{
		if (!NET_DVR_StopSearchDB(m_lHandle))
		{
			...
			g_StringLanType(szLan, "Í£Ö¹×¥ÅÄÊý¾Ý¿â²éÑ¯Ê§°Ü", "Fail to inquire");
			AfxMessageBox(szLan);
		}
		else
		{
			...
			m_lHandle = -1;
//			g_StringLanType(szLan, "Í£Ö¹×¥ÅÄÊý¾Ý¿â²éÑ¯³É¹¦", "Succ to inquire");
//			AfxMessageBox(szLan);
		}
	}
	...
	case SNAP_DATABASE_INQUIRE:
		if (m_bInquiring)
		{
			...
			if (dwState == 100)
			{
				m_bInquiring = FALSE;
				m_dwTotalRecordNum = m_lTotalNum;
				g_StringLanType(szLan, "²éÑ¯½áÊø", "Inquire complete");
				AfxMessageBox(szLan);
                if (!NET_DVR_StopSearchDB(m_lHandle))
				{
					g_pMainDlg->AddLog(m_iDevIndex, OPERATION_FAIL_T, "NET_DVR_StopSearchDB m_lHandle[%d] ",m_lHandle);
					g_StringLanType(szLan, "Í£Ö¹²éÑ¯Ê§°Ü", "Fail to inquire");
					AfxMessageBox(szLan);
				}
				else
				{
					g_pMainDlg->AddLog(m_iDevIndex, OPERATION_SUCC_T, "NET_DVR_StopSearchDB m_lHandle[%d] ",m_lHandle);
//					g_StringLanType(szLan, "Í£Ö¹²éÑ¯³É¹¦", "Succ to inquire");
//					AfxMessageBox(szLan);
					m_lHandle = -1;
				}
			}
		...
		if (m_bSearching)
		{
			...
			if (dwState == 100)
			{
				m_bSearching = FALSE;
				m_dwTotalRecordNum = m_lTotalNum;
				g_StringLanType(szLan, "¼ìË÷½áÊø", "Search complete");
				AfxMessageBox(szLan);
...
void CDlgVcaSnapDatabase::OnBtnStartSearchSnapdb() 
{
	...
	if (m_lSearchHandle < 0)
	{
        ...
        g_StringLanType(szLan, "¼ìË÷Ê§°Ü", "Fail to search record");
        AfxMessageBox(szLan);
		return;
    }
    else
...
//		g_StringLanType(szLan, "¼ìË÷³É¹¦", "Succ to search record");
//        AfxMessageBox(szLan);
    }
}
...
void CDlgVcaSnapDatabase::OnBtnDeleteSingle() 
{
	...
	if (!NET_DVR_DeleteSnapDBRecord(m_lServerID, m_dwDatabaseID, &m_struDeleteRecord))
	{
        ...
        g_StringLanType(szLan, "É¾³ý¼ÇÂ¼Ê§°Ü", "Fail to delete record");
        AfxMessageBox(szLan);
    }
    else
    {
		g_pMainDlg->AddLog(m_iDevIndex, OPERATION_SUCC_T, "NET_DVR_DeleteSnapDBRecord m_dwDatabaseID[%d] ",m_dwDatabaseID);
//		g_StringLanType(szLan, "É¾³ý¼ÇÂ¼³É¹¦", "Succ to delete record");
//        AfxMessageBox(szLan);
    }
}
...
void CDlgVcaSnapDatabase::OnBtnGetFacePic() 
{
	...
	if (!NET_DVR_FindMatchPicture(m_lServerID, &struCond, &struRecvBuf))
	{
		...
		g_StringLanType(szLan, "°´Ìõ¼þ²éÕÒ±³¾°Í¼Ê§°Ü", "Fail to find match picture");
        AfxMessageBox(szLan);
...
void CDlgVcaSnapDatabase::ProcInquireSnapCallback(WPARAM wParam, LPARAM lParam)
{
	...
	if (g_pVcaSnapDataBase->m_lRecordNum <= 2000)
	{
		...
		{
		case 0:
			g_StringLanType(szLan, "ÎÞ", "No");
			break;
		case 1:
			g_StringLanType(szLan, "ÄÐ", "Man");
			break;
		case 2:
			g_StringLanType(szLan, "Å®", "Woman");
			break;
		default:
			g_StringLanType(szLan, "Î´Öª", "Unknow");
			break;
		}
